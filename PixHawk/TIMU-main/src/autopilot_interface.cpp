...
void Autopilot_Interface::request_data_stream(uint8_t stream_id, uint16_t rate, bool start)
{
    // Initialize the message
    mavlink_message_t message;
    
    // Create the REQUEST_DATA_STREAM message
    mavlink_msg_request_data_stream_pack(
        ...
        stream_id,                  // Stream ID to request
        rate,                       // Rate in Hz
        start ? 1 : 0               // 1 to start, 0 to stop
    );
    
    // Send the message
    linker->write_message(message);
